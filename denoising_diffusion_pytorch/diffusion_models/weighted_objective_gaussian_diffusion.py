...
class WeightedObjectiveGaussianDiffusion(GaussianDiffusion):
    def __init__(
        ...
        assert model.out_dim == (
                    channels * 2 + 2), 'dimension out (out_dim) of unet must be twice the number of channels + 2 (for the softmax weighted sum) - for channels of 3, this should be (3 * 2) + 2 = 8'
    ...
    def p_mean_variance(self, *, x, t, clip_denoised, model_output=None):
        ...
        x_start_from_noise = self.predict_x0_from_noise(x, t=t, noise=pred_noise)
    ...
    def p_losses(self, x_start, t, noise=None, clip_denoised=False):
        ...
        x_start_from_pred_noise = self.predict_x0_from_noise(x_t, t, pred_noise)
        x_start_from_pred_noise = x_start_from_pred_noise.clamp(-2., 2.)
        weighted_x_start = einsum('b j h w, b j c h w -> b c h w', weights.softmax(dim=1),
